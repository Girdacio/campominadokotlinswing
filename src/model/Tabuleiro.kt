...
class Tabuleiro(val qtdeLinhas: Int, val qtdeColunas: Int, private val qtdeMinas: Int) {
    ...
    private fun objetivoAlcancado(): Boolean {
        ...
        var qtdeRestante: Int = 0;
        forEachCampo { if (it.fechado) qtdeRestante++ else qtdeRestante }

        var restaramSomenteBombas = qtdeRestante == qtdeMinas

        return jogadorGanhou || restaramSomenteBombas
    }
    ...
    private fun gerarCampos() {
        for (linha in 0 until qtdeLinhas) {  // (until) até qtdeLinhas - 1
            campos.add(ArrayList())
            for (coluna in 0 until qtdeColunas) {   // (until) até qtdeColunas - 1
